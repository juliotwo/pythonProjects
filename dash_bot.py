...
def putOrder(asks,bids):
    ...
    firstbid= bids[0]
    priceBid=firstbid["p"]
    url="https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=dash"
    print(url)
    r = requests.get(url)
    data= r.json()
    payload=data[0]

    urlUsd="https://api.bitso.com/v3/ticker/?book=tusd_mxn"
    print(urlUsd)
    r = requests.get(urlUsd)
    dataUsd= json.loads(r.text)
    payloadUsd=dataUsd["payload"]
    lastPriceUd= payloadUsd["last"]

    lastPrice= float(payload["current_price"])*float(lastPriceUd)
    print("Last Price bitso",lastPrice)
    # lastPrice= 1190000
    priceToPutSell=float(priceAsk)-1
    sellExists = validOrderExistsSell(priceAsk)
    ...
    if sellExists==False and priceToPutSell > float(lastPrice) and percentageSell>limitPercentage :
        print("Orden")
        ...
        try:
            ...
            if percentageToClose<limitPercentage:
                closeAllOrders(filterOrdersSell)
        except:
            print("NO EXISTE")
    ...
    priceToPutBuy=float(priceBid)+1
    filterOrdersBuy=list(filter(lambda x: x["side"] =="BUY", orders))
    buyExists = validOrderExistsBuy(priceBid,filterOrdersBuy)
    ...
    if buyExists==False and priceToPutBuy < float(lastPrice) and percentageBuy>limitPercentage :
